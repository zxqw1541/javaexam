...
public class ProjectClient {
  class QueryString {
    ...
    public QueryString(String str) { 
      //str 형식
      // 예1) 192.168.1.2:8080/project/list
      // 예2) 192.168.1.2/project/list
      // 예3) 192.168.1.2:8080/project/add?title=aaa&startDate=...
      // 예4) 192.168.1.2/project/add?title=aaa&startDate=...
      
      //1) 서버 주소 뜯어내기
      // - 사용자가 임력한 문자열을 앞에서부터 / 문자가 있는 위치를 찾는다.
      int i = str.indexOf('/');
      // - 앞에서 부터 / 문자 전까지 문자열을 추출한다. 
  ...
  private void send(QueryString qs) {
        ...
        )
    {
      ...
      } while (!msg.equals("")); // 빈 줄을 만날 때 까지 반복한다.
    } catch (Exception e) { e.printStackTrace();}
  }
