 ...
 * => doFilter(): 다음 필터로 가기전에 호출됨. 필터가 해야할 작업을 작성한다.
 * => 웹 애플리케이션이 시작되면 web.xml 등록된 필터 객체를 자동 생성한다.
 */
package filter;
...
public class Filter03 implements Filter {
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    System.out.println("Filter03.doFilter()... 전반 작업");
    
    //다음 필터를 실행한다. 만약 다음 필터가 없다면, 서블릿이 실행된다.
    chain.doFilter(request, response);
    
    System.out.println("Filter03.doFilter()... 후반 작업");
  }
