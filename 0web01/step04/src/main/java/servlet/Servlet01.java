...
=> Servlet 인터페이스
   init() - 서블릿 실행을 최초로 요청 받으면, 해당 클래스의 인스턴스를 생성한 후 호출한다.
           .서블릿이 작업하는데 필요한 준비를 자원을 준비하는 코드를 넣으면 된다.
           .딱 한 번 호출 된다.
            *.* => 서블릿은 오직 한 개만 생성된다.
...
public class Servlet01 implements Servlet {
  ...
  public void init(ServletConfig config) throws ServletException {
    ...
    /*
     * ServletConfig 파라미터? => 서블릿 설정 정보를 다룰 때 사용하는 도구이다. => 웹 애플리케이션 정보를 다루는 도구도
     * 알아 낼 수 있다. => 이 도구를 이용하여 웹 어플리케이션 정보를 다루는 도구를 얻을 수 있다. => 그래서 나중에 사용할 것을
     * 대비해 인스턴스 변수에 저장해 두는 것이 좋다.
     */
    this.config = config;
  }
  ...
  public void service(ServletRequest request, ServletResponse response) throws ServletException, IOException {
    ...
    /*
     * ServletRequest 파라미터? => 요청 정보를 다룰 때 사용하는 도구 => 클라이언트가 보낸 값을 꺼낼 수 있다. =>
     * 요청한 클라이언트의 IP 주소를 알아낼 수 있다. => 클라이언트의 웹 브라우저 및 운영체제 정보를 알아낼 수 있다.
     * 
     * ServletResponse 파라미터? => 응답을 다룰 때 사용하는 도구 => 클라이언트에게 출력할 때 사용할 출력 스트림 객체를
     * 얻을 수 있다. => 클라이언트에게 보내는 데이터의 유형과 문자 집합을 지정할 수 있다.
     */
  ...
  public ServletConfig getServletConfig() {
    System.out.println("getServletifg()");
    return config;
  }
