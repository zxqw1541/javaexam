...
public class AnnotationApplicationContext01 {
  ...
  public AnnotationApplicationContext01(String basePackage) throws Exception {
    createObject(basePackage, new File("./bin/" + basePackage.replace(".", "/")));
    injectDependcies();
  }
  ...
  private void createObject(String packageName, File file) throws Exception {
    ...
    for (File f : subFiles) {
      if (f.isDirectory()) {
        createObject(packageName + "." + f.getName(), f);
        continue;
      }
      //클래스 파일을 로딩한다.
