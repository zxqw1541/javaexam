  ...
  protected void service(
    ...
    try {
      ...
      for (int i = 0; i < params.length; i++) {
        //파라미터 명을 보기 위해 window->preference ->java->compiler -> store... 체크
          ...
          paramValues[i] = new HashMap<String, Object>();
            ...
            try{
              paramValues[i] = Integer.parseInt(
                  (String)getParameter(params[i].getName(), request, multipartParamMap));
            } catch (Exception e) {
              paramValues[i] = -1;
            }
        }         
